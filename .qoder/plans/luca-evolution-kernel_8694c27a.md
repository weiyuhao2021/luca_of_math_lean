...
- 设计 6 种天灾类型，概率驱动（不看语义、不看死活）：
  - 触发公式：\\( P(\text{cataclysm}) = p_0 \times (\frac{\text{urn\_size}}{\text{MAX\_URN\_SIZE}})^\alpha \\)，瓮越臃肿灾变风险越高
  - 参数：\\( p_0 = 0.005 \\)，\\( \alpha = 2.0 \\)，最小间隔 20 代
  - 天灾类型（均匀随机选择）：
    - **质量灭绝** (`mass_extinction`)：移除 80%-90% 的低权重 token
    - **中等灭绝** (`moderate_extinction`)：移除 40%-60% + 幸存者权重增强 1.1-1.5x
    - **基因漂变** (`genetic_drift`)：所有权重向均值回归 30%
    - **寒武纪大爆发** (`cambrian_explosion`)：高权重 token 两两杂交生成新复合 token
    - **小行星撞击** (`asteroid_impact`)：随机删除 50% token，幸存者权重翻倍
    - **冰河期** (`ice_age`)：权重减半，抽象构造 token 获抗冻加成

## 11. 数据持久化
- 成功编译的 Lean 定义追加写入 `evolved_library.lean`——同时也是每次编译的祖先上下文
- 定期保存瓮的状态、谱系污染集合、纪元/多细胞统计到 JSON（支持断点续跑）
- 演化日志保存到 `evolution_log.jsonl`

## 12. 进化事件追踪
- **谱系追踪**：含 `sorry` 的基因标记为公理化谱系，后代若引用亦继承污染
  - `[ALIVE]` = 封闭谱系（仅依赖 Init + 无 sorry 祖先）
  - `[ALIVE★]` = 公理化谱系（含或依赖 sorry，本质是注入新公理）
- **多细胞化检测**：新基因引用其他 `gene_N` 时，终端输出 `🧬 多细胞化!` 及共生基因列表
  - 基因引用已可真实编译——候选代码在 `evolved_library.lean` 的全量祖先环境下验证
